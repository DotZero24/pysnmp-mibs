#
# PySNMP MIB module ALGPOWER_v2-MIB (http://snmplabs.com/pysmi)
# ASN.1 source file:///Users/rob/Code/pysnmp-mibs/mibs/algcom/ALGPOWER_v2-MIB
...
bat_tst = MibIdentifier((1, 3, 6, 1, 4, 1, 49136, 1, 8))
...
btst_date = MibScalar((1, 3, 6, 1, 4, 1, 49136, 1, 8, 1), DisplayString().subtype(subtypeSpec=ValueSizeConstraint(10, 10)).setFixedLength(10)).setMaxAccess("readonly")
if mibBuilder.loadTexts: btst_date.setStatus('mandatory')
btst_status = MibScalar((1, 3, 6, 1, 4, 1, 49136, 1, 8, 2), Integer32()).setMaxAccess("readonly")
if mibBuilder.loadTexts: btst_status.setStatus('mandatory')
btst_duration = MibScalar((1, 3, 6, 1, 4, 1, 49136, 1, 8, 3), Integer32()).setMaxAccess("readonly")
if mibBuilder.loadTexts: btst_duration.setStatus('mandatory')
btst_elapsed = MibScalar((1, 3, 6, 1, 4, 1, 49136, 1, 8, 4), Integer32()).setMaxAccess("readonly")
if mibBuilder.loadTexts: btst_elapsed.setStatus('mandatory')
btst_volt_i = MibScalar((1, 3, 6, 1, 4, 1, 49136, 1, 8, 5), Integer32()).setMaxAccess("readonly")
if mibBuilder.loadTexts: btst_volt_i.setStatus('mandatory')
btst_amp_i = MibScalar((1, 3, 6, 1, 4, 1, 49136, 1, 8, 6), Integer32()).setMaxAccess("readonly")
if mibBuilder.loadTexts: btst_amp_i.setStatus('mandatory')
btst_volt_f = MibScalar((1, 3, 6, 1, 4, 1, 49136, 1, 8, 7), Integer32()).setMaxAccess("readonly")
if mibBuilder.loadTexts: btst_volt_f.setStatus('mandatory')
btst_amp_f = MibScalar((1, 3, 6, 1, 4, 1, 49136, 1, 8, 8), Integer32()).setMaxAccess("readonly")
if mibBuilder.loadTexts: btst_amp_f.setStatus('mandatory')
mibBuilder.exportSymbols("ALGPOWER_v2-MIB", oidNotUsed0=oidNotUsed0, facilityAddr=facilityAddr, watchdogPing2=watchdogPing2, installation=installation, watchdogPing10=watchdogPing10, btst_date=btst_date, upsObjects=upsObjects, watchdogPing8=watchdogPing8, upTime=upTime, watchdogPing4=watchdogPing4, overload=overload, batteryCharging=batteryCharging, bat_tst=bat_tst, outputCurrent=outputCurrent, btst_elapsed=btst_elapsed, popName=popName, oidNotUsed3=oidNotUsed3, btst_duration=btst_duration, respPers=respPers, btst_volt_f=btst_volt_f, oidNotUsed2=oidNotUsed2, watchdogPing7=watchdogPing7, output=output, supply=supply, algcom=algcom, watchdogPing1=watchdogPing1, btst_status=btst_status, watchdogPing6=watchdogPing6, batInstDate=batInstDate, watchdogPing9=watchdogPing9, instDate=instDate, batBrand=batBrand, batteryDischarging=batteryDischarging, acFail=acFail, input=input, supplyVoltage=supplyVoltage, btst_volt_i=btst_volt_i, btst_amp_i=btst_amp_i, heatSinkTemperature=heatSinkTemperature, watchdogPing3=watchdogPing3, temperature=temperature, chargerStatus=chargerStatus, fanBfail=fanBfail, overheat=overheat, fanAfail=fanAfail, alarmOnBattery=alarmOnBattery, facilityCity=facilityCity, watchdogPing5=watchdogPing5, watchdog=watchdog, batteryVoltage=batteryVoltage, outputVoltage=outputVoltage, oidNotUsed1=oidNotUsed1, batCapacity=batCapacity, btst_amp_f=btst_amp_f, innerTemperature=innerTemperature, batteryCurrent=batteryCurrent, outerTemperature=outerTemperature, battery=battery)
