...
# ASN.1 source file:///Users/rob/Code/pysnmp-mibs/mibs/cisco/CISCO-CEF-MIB
...
CefForwardingElementSpecialType, CefOperStatus, CefIpVersion, CefPathType, CefAdjacencySource, CefCCType, CefPrefixSearchState, CefCCStatus, CefMplsLabelList, CefAdminStatus, CefFailureReason, CefCCAction, CefAdjLinkType = mibBuilder.importSymbols("CISCO-CEF-TC", "CefForwardingElementSpecialType", "CefOperStatus", "CefIpVersion", "CefPathType", "CefAdjacencySource", "CefCCType", "CefPrefixSearchState", "CefCCStatus", "CefMplsLabelList", "CefAdminStatus", "CefFailureReason", "CefCCAction", "CefAdjLinkType")
...
mibBuilder.exportSymbols("CISCO-CEF-MIB", cefPeerOperState=cefPeerOperState, cefInconsistencyReason=cefInconsistencyReason, ciscoCefMIBNotifs=ciscoCefMIBNotifs, cefPrefixHCBytes=cefPrefixHCBytes, cefResourceTable=cefResourceTable, cefPrefixExternalNRBytes=cefPrefixExternalNRBytes, cefFESelectionWeight=cefFESelectionWeight, cefCfgLoadSharingID=cefCfgLoadSharingID, cefStatsPrefixInserts=cefStatsPrefixInserts, cefCCQueriesChecked=cefCCQueriesChecked, cefInconsistencyVrfName=cefInconsistencyVrfName, cefGroup=cefGroup, cefNotificationGroup=cefNotificationGroup, entPeerPhysicalIndex=entPeerPhysicalIndex, cefFESelectionVrfName=cefFESelectionVrfName, cefAdj=cefAdj, cefStatsPrefixLen=cefStatsPrefixLen, cefPrefixEntry=cefPrefixEntry, cefAdjEncap=cefAdjEncap, cefAdjSource=cefAdjSource, cefResourceMemoryUsed=cefResourceMemoryUsed, ciscoCefMIBConform=ciscoCefMIBConform, cefSwitchingPunt=cefSwitchingPunt, cefStats=cefStats, cefStatsPrefixHCDeletes=cefStatsPrefixHCDeletes, cefCCEnabled=cefCCEnabled, cefLMPrefixTable=cefLMPrefixTable, cefInconsistencyPrefixType=cefInconsistencyPrefixType, cefPrefixExternalNRPkts=cefPrefixExternalNRPkts, cefPrefixInternalNRBytes=cefPrefixInternalNRBytes, cefLMPrefixLen=cefLMPrefixLen, cefFESelectionId=cefFESelectionId, cefFIBSummaryFwdPrefixes=cefFIBSummaryFwdPrefixes, cefAdjSummary=cefAdjSummary, cefFIBSummary=cefFIBSummary, cefPathType=cefPathType, cefAdjSummaryTable=cefAdjSummaryTable, cefPrefixType=cefPrefixType, PYSNMP_MODULE_ID=ciscoCefMIB, cefLMPrefixEntry=cefLMPrefixEntry, cefPathEntry=cefPathEntry, cefAdjSummaryFixup=cefAdjSummaryFixup, cefIntNonrecursiveAccouting=cefIntNonrecursiveAccouting, cefPeerTable=cefPeerTable, cefInconsistencyEntity=cefInconsistencyEntity, cefStatsPrefixHCInserts=cefStatsPrefixHCInserts, cefInconsistencyRecordTable=cefInconsistencyRecordTable, cefPeerStateChange=cefPeerStateChange, cefPrefixInternalNRPkts=cefPrefixInternalNRPkts, cefCfgTrafficStatsUpdateRate=cefCfgTrafficStatsUpdateRate, cefPathRecurseVrfName=cefPathRecurseVrfName, cefPrefixBytes=cefPrefixBytes, cefPathNextHopAddr=cefPathNextHopAddr, cefFIB=cefFIB, cefInconsistencyReset=cefInconsistencyReset, cefPeerFIBStateChangeNotifEnable=cefPeerFIBStateChangeNotifEnable, cefAdjHCBytes=cefAdjHCBytes, cefPrefixAddr=cefPrefixAddr, cefLMPrefixSpinLock=cefLMPrefixSpinLock, cefStatsPrefixHCQueries=cefStatsPrefixHCQueries, cefIntLoadSharing=cefIntLoadSharing, cefPrefixInternalNRHCPkts=cefPrefixInternalNRHCPkts, cefSwitchingPunt2Host=cefSwitchingPunt2Host, cefPrefixLen=cefPrefixLen, cefAdjConnId=cefAdjConnId, cefCCQueriesIgnored=cefCCQueriesIgnored, cefLMPrefixState=cefLMPrefixState, cefAdjSummaryEntry=cefAdjSummaryEntry, cefAdjNextHopAddrType=cefAdjNextHopAddrType, cefCCTypeTable=cefCCTypeTable, cefFESelectionSpecial=cefFESelectionSpecial, cefPathTable=cefPathTable, cefMIBCompliances=cefMIBCompliances, cefCCTypeEntry=cefCCTypeEntry, cefMIBCompliance=cefMIBCompliance, cefResourceFailure=cefResourceFailure, cefCCGlobalAutoRepairDelay=cefCCGlobalAutoRepairDelay, cefInconsistencyDetection=cefInconsistencyDetection, cefPrefixForwardingInfo=cefPrefixForwardingInfo, ciscoCefMIB=ciscoCefMIB, cefInconsistencyRecordEntry=cefInconsistencyRecordEntry, cefSwitchingPath=cefSwitchingPath, cefCCGlobalFullScanAction=cefCCGlobalFullScanAction, entLastInconsistencyDetectTime=entLastInconsistencyDetectTime, cefNotifThrottlingInterval=cefNotifThrottlingInterval, cefPrefixExternalNRHCPkts=cefPrefixExternalNRHCPkts, cefGlobal=cefGlobal, cefPeer=cefPeer, cefFIBSummaryEntry=cefFIBSummaryEntry, cefAdjForwardingInfo=cefAdjForwardingInfo, cefFE=cefFE, cefNotifCntlGroup=cefNotifCntlGroup, cefCC=cefCC, cefFESelectionName=cefFESelectionName, cefPathId=cefPathId, cefFESelectionLabels=cefFESelectionLabels, cefPrefixInternalNRHCBytes=cefPrefixInternalNRHCBytes, cefCCType=cefCCType, cefCfgAccountingMap=cefCfgAccountingMap, cefFESelectionAdjLinkType=cefFESelectionAdjLinkType, cefLMPrefixRowStatus=cefLMPrefixRowStatus, cefLMPrefixDestAddr=cefLMPrefixDestAddr, cefAdjSummaryIncomplete=cefAdjSummaryIncomplete, cefLMPrefixDestAddrType=cefLMPrefixDestAddrType, cefStatsPrefixHCElements=cefStatsPrefixHCElements, cefCfgAdminState=cefCfgAdminState, cefAdjBytes=cefAdjBytes, cefPeerEntry=cefPeerEntry, cefStatsPrefixQueries=cefStatsPrefixQueries, cefInconsistencyRecId=cefInconsistencyRecId, cefMIBGroups=cefMIBGroups, cefInconsistencyCCType=cefInconsistencyCCType, cefInterface=cefInterface, cefAdjFixup=cefAdjFixup, cefCfgTrafficStatsLoadInterval=cefCfgTrafficStatsLoadInterval, cefAdjSummaryRedirect=cefAdjSummaryRedirect, cefStatsPrefixDeletes=cefStatsPrefixDeletes, cefCfgOperState=cefCfgOperState, cefCCQueriesSent=cefCCQueriesSent, cefPeerFIBStateChange=cefPeerFIBStateChange, cefCCGlobalTable=cefCCGlobalTable, cefSwitchingHCPunt=cefSwitchingHCPunt, cefCCGlobalErrorMsgEnabled=cefCCGlobalErrorMsgEnabled, cefResourceEntry=cefResourceEntry, cefAdjHCPkts=cefAdjHCPkts, cefFESelectionAdjNextHopAddrType=cefFESelectionAdjNextHopAddrType, cefResourceFailureReason=cefResourceFailureReason, cefCCCount=cefCCCount, cefPeerFIBOperState=cefPeerFIBOperState, cefAdjEntry=cefAdjEntry, cefCfgLoadSharingAlgorithm=cefCfgLoadSharingAlgorithm, cefAdjSummaryComplete=cefAdjSummaryComplete, cefInconsistencyPrefixLen=cefInconsistencyPrefixLen, cefFESelectionAdjConnId=cefFESelectionAdjConnId, cefLMPrefixAddr=cefLMPrefixAddr, ciscoCefMIBObjects=ciscoCefMIBObjects, cefPrefixTable=cefPrefixTable, cefFIBSummaryTable=cefFIBSummaryTable, cefPeerFIBTable=cefPeerFIBTable, cefAdjTable=cefAdjTable, cefSwitchingIndex=cefSwitchingIndex, cefInconsistencyPrefixAddr=cefInconsistencyPrefixAddr, cefCfgDistributionAdminState=cefCfgDistributionAdminState, cefFESelectionAdjNextHopAddr=cefFESelectionAdjNextHopAddr, cefPathInterface=cefPathInterface, cefInconsistencyResetStatus=cefInconsistencyResetStatus, cefPrefixHCPkts=cefPrefixHCPkts, cefSwitchingHCDrop=cefSwitchingHCDrop, cefSwitchingDrop=cefSwitchingDrop, cefPeerFIBEntry=cefPeerFIBEntry, cefAdjPkts=cefAdjPkts, cefFESelectionTable=cefFESelectionTable, cefSwitchingStatsEntry=cefSwitchingStatsEntry, cefPrefixExternalNRHCBytes=cefPrefixExternalNRHCBytes, cefFIBIpVersion=cefFIBIpVersion, cefCfgTable=cefCfgTable, cefStatsPrefixLenEntry=cefStatsPrefixLenEntry, cefSwitchingStatsTable=cefSwitchingStatsTable, cefCCGlobalAutoRepairHoldDown=cefCCGlobalAutoRepairHoldDown, cefCCGlobalEntry=cefCCGlobalEntry, cefSwitchingHCPunt2Host=cefSwitchingHCPunt2Host, cefNotifCntl=cefNotifCntl, cefPeerStateChangeNotifEnable=cefPeerStateChangeNotifEnable, cefIntTable=cefIntTable, cefCCGlobalAutoRepairEnabled=cefCCGlobalAutoRepairEnabled, cefStatsPrefixLenTable=cefStatsPrefixLenTable, cefStatsPrefixElements=cefStatsPrefixElements, cefCCGlobalFullScanStatus=cefCCGlobalFullScanStatus, cefFESelectionAdjInterface=cefFESelectionAdjInterface, cefDistributedGroup=cefDistributedGroup, cefFESelectionEntry=cefFESelectionEntry, cefCCQueriesIterated=cefCCQueriesIterated, cefCfgEntry=cefCfgEntry, cefCfgDistributionOperState=cefCfgDistributionOperState, cefPeerNumberOfResets=cefPeerNumberOfResets, cefHCStatsGroup=cefHCStatsGroup, cefCCPeriod=cefCCPeriod, cefAdjSummaryLinkType=cefAdjSummaryLinkType, cefAdjNextHopAddr=cefAdjNextHopAddr, cefAdjMTU=cefAdjMTU, cefResourceFailureNotifEnable=cefResourceFailureNotifEnable, cefInconsistencyNotifEnable=cefInconsistencyNotifEnable, cefIntEntry=cefIntEntry, cefIntSwitchingState=cefIntSwitchingState, cefPrefixPkts=cefPrefixPkts)
