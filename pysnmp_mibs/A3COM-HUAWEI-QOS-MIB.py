...
#
vrpProtocol, hwLocal, hwInternetProtocol, huawei = mibBuilder.importSymbols("A3COM-HUAWEI-OID-MIB", "vrpProtocol", "hwLocal", "hwInternetProtocol", "huawei")
...
mibBuilder.exportSymbols("A3COM-HUAWEI-QOS-MIB", qosCQRunInfoIfIndex=qosCQRunInfoIfIndex, qosGTSIfIndex=qosGTSIfIndex, qosPQIfIndex=qosPQIfIndex, qosUndoPQ=qosUndoPQ, qosCqlListNum=qosCqlListNum, qosCqlQueueNum=qosCqlQueueNum, qosGTSBurstSize=qosGTSBurstSize, qosUndoGTS=qosUndoGTS, qosLRBurstSize=qosLRBurstSize, qosFIFOIfName=qosFIFOIfName, qosCARIfName=qosCARIfName, qosWREDEnable=qosWREDEnable, qosWREDIfIndex=qosWREDIfIndex, qosLRExcessBurstSize=qosLRExcessBurstSize, qosPqlQueueLenTable=qosPqlQueueLenTable, qosUndoLR=qosUndoLR, qosCQRunInfoQuePkt=qosCQRunInfoQuePkt, qosPqlIfQueueType=qosPqlIfQueueType, qosGTSExcessBurstSize=qosGTSExcessBurstSize, qosRTPIfQueueOutputs=qosRTPIfQueueOutputs, qosWFQCurQueLen=qosWFQCurQueLen, qosCqlIfEntry=qosCqlIfEntry, qosCqlIfListNum=qosCqlIfListNum, qosCqlIfQueueNum=qosCqlIfQueueNum, qosCqlQueParaQueNum=qosCqlQueParaQueNum, qosCQIfName=qosCQIfName, qosRTPIfQueueRunInfoTable=qosRTPIfQueueRunInfoTable, qosCQTable=qosCQTable, qosCarlTable=qosCarlTable, qosCqlProQueKey=qosCqlProQueKey, qosCQListNum=qosCQListNum, qosPQTopDiscard=qosPQTopDiscard, qosCarlParaValue=qosCarlParaValue, qosWFQMaxActiveQueNum=qosWFQMaxActiveQueNum, qosPQIfName=qosPQIfName, qosPQMiddlePkt=qosPQMiddlePkt, qosCqlProQueNum=qosCqlProQueNum, qosCqlProtocolTable=qosCqlProtocolTable, qosWREDWeightConstant=qosWREDWeightConstant, qosFIFOEntry=qosFIFOEntry, qosPqlProQueKeyValue=qosPqlProQueKeyValue, qosPqlIfTable=qosPqlIfTable, qosRTPIfApplyBandWidth=qosRTPIfApplyBandWidth, qosLRIfIndex=qosLRIfIndex, qosPqlIfIndex=qosPqlIfIndex, qosCARIfIndex=qosCARIfIndex, qosWREDPreDiscardProbability=qosWREDPreDiscardProbability, qosWREDPreLowLimit=qosWREDPreLowLimit, qosCarlListNum=qosCarlListNum, qosCARCIR=qosCARCIR, qosPqlIfName=qosPqlIfName, qosCARExceedNewPrec=qosCARExceedNewPrec, qosPqlProQueKey=qosPqlProQueKey, qosCARExceedPkt=qosCARExceedPkt, qosWREDPreHighLimit=qosWREDPreHighLimit, qosFIFOIfIndex=qosFIFOIfIndex, qosUndoFIFO=qosUndoFIFO, qosIFBandwidthMaxBW=qosIFBandwidthMaxBW, qosPqlProListNum=qosPqlProListNum, qosCqlProQueKeyValue=qosCqlProQueKeyValue, qosGTSEntry=qosGTSEntry, qosCqlQueParaServing=qosCqlQueParaServing, qosCQIfIndex=qosCQIfIndex, qosRTPIfApplyCbs=qosRTPIfApplyCbs, qosUndoPqlProtocol=qosUndoPqlProtocol, qosRTPIfApplyIfIndex=qosRTPIfApplyIfIndex, qosWFQTable=qosWFQTable, qosWFQTotalQueNum=qosWFQTotalQueNum, qosGTSType=qosGTSType, qosCARConformNewPrec=qosCARConformNewPrec, qosCARExceedByte=qosCARExceedByte, qosUndoWREDPre=qosUndoWREDPre, qosCQRunInfoEntry=qosCQRunInfoEntry, qosIFBandwidthAvailable=qosIFBandwidthAvailable, qosCqlDefaultEntry=qosCqlDefaultEntry, qosWFQTotalDiscard=qosWFQTotalDiscard, qosPQListNum=qosPQListNum, qosWFQActiveQueNum=qosWFQActiveQueNum, qosPqlProName=qosPqlProName, qosUndoCAR=qosUndoCAR, qosGTSCIR=qosGTSCIR, qosWFQIfName=qosWFQIfName, qosRTPIfApplyTable=qosRTPIfApplyTable, qosWREDPreRandomDropNum=qosWREDPreRandomDropNum, qosIFBandwidthMaxReservedBW=qosIFBandwidthMaxReservedBW, qosIFBandwidthMaxReservedBWPct=qosIFBandwidthMaxReservedBWPct, qosPQEntry=qosPQEntry, qosPQTopPkt=qosPQTopPkt, qosUndoCqlDefault=qosUndoCqlDefault, qosCqlProName=qosCqlProName, qosRTPIfQueueSize=qosRTPIfQueueSize, qosLRCIR=qosLRCIR, qosLRPassByte=qosLRPassByte, qosUndoWREDWeightConstant=qosUndoWREDWeightConstant, qosUndoCarl=qosUndoCarl, qosGTSDiscardPkt=qosGTSDiscardPkt, qosGTSCurQueLen=qosGTSCurQueLen, qosPQMiddleMaxQueLen=qosPQMiddleMaxQueLen, qosLREntry=qosLREntry, qosPQMiddleDiscard=qosPQMiddleDiscard, qosPQBottomMaxQueLen=qosPQBottomMaxQueLen, qosUndoPqlDefault=qosUndoPqlDefault, qosCqlQueParaMaxQueLen=qosCqlQueParaMaxQueLen, qosRTPIfQueueRunInfoEntry=qosRTPIfQueueRunInfoEntry, qosPqlDefaultEntry=qosPqlDefaultEntry, qosIFBandwidthRowStatus=qosIFBandwidthRowStatus, qosWREDIfName=qosWREDIfName, qosUndoWFQ=qosUndoWFQ, qosIfBandwidthIfIndex=qosIfBandwidthIfIndex, qosGTSMaxQueLen=qosGTSMaxQueLen, qosRTPIfQueueMaxSize=qosRTPIfQueueMaxSize, qosWREDTable=qosWREDTable, qosPQNormalPkt=qosPQNormalPkt, qosFIFOTable=qosFIFOTable, qosPqlIfListNum=qosPqlIfListNum, qosWREDPreTailDropNum=qosWREDPreTailDropNum, qosUndoPqlIf=qosUndoPqlIf, qosCqlQueParaEntry=qosCqlQueParaEntry, qosGTSACLNum=qosGTSACLNum, qosWREDPrecedence=qosWREDPrecedence, qosPQTable=qosPQTable, qosLRPassPkt=qosLRPassPkt, qosPqlProtocolTable=qosPqlProtocolTable, qosIfBandwidthEntry=qosIfBandwidthEntry, qosGTSDiscardByte=qosGTSDiscardByte, qosGTSPassByte=qosGTSPassByte, qosCQRunInfoMaxQueLen=qosCQRunInfoMaxQueLen, qosRTPIfApplyEndPort=qosRTPIfApplyEndPort, PYSNMP_MODULE_ID=hwIfQoSMib, qosCARConformAction=qosCARConformAction, qosUndoCqlQueParaMaxQueLen=qosUndoCqlQueParaMaxQueLen, qosCarlParaType=qosCarlParaType, qosWREDEntry=qosWREDEntry, qosWREDPreIfName=qosWREDPreIfName, qosCARListNum=qosCARListNum, qosLRDelayByte=qosLRDelayByte, qosCARConformPkt=qosCARConformPkt, qosWREDPreIfIndex=qosWREDPreIfIndex, qosIfBandwidthTable=qosIfBandwidthTable, qosPqlQueLenListNum=qosPqlQueLenListNum, qosPqlDefaultTable=qosPqlDefaultTable, qosCARConformByte=qosCARConformByte, qosGTSDelayByte=qosGTSDelayByte, qosLRTable=qosLRTable, qosCqlProListNum=qosCqlProListNum, qosPqlQueueLenEntry=qosPqlQueueLenEntry, qosCqlQueParaListNum=qosCqlQueParaListNum, qosCARExceedAction=qosCARExceedAction, qosRTPIfApplyStartPort=qosRTPIfApplyStartPort, qosRTPIfApplyEntry=qosRTPIfApplyEntry, qosCARType=qosCARType, qosPqlIfEntry=qosPqlIfEntry, qosUndoCQ=qosUndoCQ, qosCqlIfTable=qosCqlIfTable, qosPQNormalDiscard=qosPQNormalDiscard, qosCQRunInfoTable=qosCQRunInfoTable, qosPQNormalMaxQueLen=qosPQNormalMaxQueLen, qosCarlEntry=qosCarlEntry, qosWREDPreTable=qosWREDPreTable, qosPqlDefaultQueueType=qosPqlDefaultQueueType, qosCqlIfName=qosCqlIfName, qosFIFOCurQueueLen=qosFIFOCurQueueLen, qosUndoPqlQueLen=qosUndoPqlQueLen, qosCqlProtocolEntry=qosCqlProtocolEntry, qosPqlQueLenQueueType=qosPqlQueLenQueueType, qosCQRunInfoQueDiscard=qosCQRunInfoQueDiscard, qosWFQQueueType=qosWFQQueueType, qosGTSIfName=qosGTSIfName, qosCARBurstSize=qosCARBurstSize, qosWFQEntry=qosWFQEntry, qosGTSDelayPkt=qosGTSDelayPkt, qosGTSTable=qosGTSTable, qosWFQIfIndex=qosWFQIfIndex, qosUndoCqlIf=qosUndoCqlIf, qosLRDelayPkt=qosLRDelayPkt, qosPqlProQueType=qosPqlProQueType, qosPQBottomPkt=qosPQBottomPkt, qosCQRunInfoQueNum=qosCQRunInfoQueNum, qosCqlIfIndex=qosCqlIfIndex, qosUndoCqlQueParaServing=qosUndoCqlQueParaServing, qosCARExcessBurstSize=qosCARExcessBurstSize, hwIfQoSMib=hwIfQoSMib, qosCARTable=qosCARTable, qosRTPIfQueueDiscards=qosRTPIfQueueDiscards, qosLRIfName=qosLRIfName, qosPqlDefaultListNum=qosPqlDefaultListNum, qosGTSPassPkt=qosGTSPassPkt, qosPQBottomDiscard=qosPQBottomDiscard, qosFIFODiscardPkt=qosFIFODiscardPkt, qosRTPIfApplyRowStatus=qosRTPIfApplyRowStatus, qosCQEntry=qosCQEntry, qosWFQMaxQueLen=qosWFQMaxQueLen, qosPqlQueLenValue=qosPqlQueLenValue, qosWREDPreEntry=qosWREDPreEntry, qosFIFOMaxQueueLen=qosFIFOMaxQueueLen, qosUndoCqlProtocol=qosUndoCqlProtocol, qosCQRunInfoIfName=qosCQRunInfoIfName, qosPQTopMaxQueLen=qosPQTopMaxQueLen, qosCARPktDirection=qosCARPktDirection, qosCqlDefaultTable=qosCqlDefaultTable, qosCAREntry=qosCAREntry, qosPqlProtocolEntry=qosPqlProtocolEntry, qosCqlQueParaTable=qosCqlQueParaTable)
