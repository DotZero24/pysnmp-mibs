...
# ASN.1 source file:///Users/rob/Code/pysnmp-mibs/mibs/gcom/GBNPlatformOAM-MIB
...
gbnPlatformOAM.setRevisions(('2018-02-14 00:00', '1900-11-02 00:00',))
if mibBuilder.loadTexts: gbnPlatformOAM.setLastUpdated('201802140000Z')
...
class VctRunResultStatusType(TextualConvention, Integer32):
    status = 'current'
    subtypeSpec = Integer32.subtypeSpec + ConstraintsUnion(SingleValueConstraint(0, 1, 2, 3))
    namedValues = NamedValues(("normal", 0), ("short", 1), ("open", 2), ("impedance-mismatch", 3))
...
mibBuilder.exportSymbols("GBNPlatformOAM-MIB", memorySize=memorySize, loadFtpUserPassword=loadFtpUserPassword, snmpCommunityToViewEntry=snmpCommunityToViewEntry, vctRunResultStatus=vctRunResultStatus, softDogProxy=softDogProxy, vctRunResultEntry=vctRunResultEntry, portCarPort=portCarPort, snmpPrivateNotifyType=snmpPrivateNotifyType, vctAutoRunGlobalEnable=vctAutoRunGlobalEnable, loadTftpFileName=loadTftpFileName, snmpComm2ViewCommName=snmpComm2ViewCommName, PYSNMP_MODULE_ID=gbnPlatformOAM, clockTime=clockTime, cpuBusyTrap=cpuBusyTrap, vctRunResultTable=vctRunResultTable, sysIfDHCPOnOff=sysIfDHCPOnOff, prodSerialNo=prodSerialNo, loadType=loadType, gbnPlatformOAMSystem=gbnPlatformOAMSystem, timeZoneOffset=timeZoneOffset, portCarEnable=portCarEnable, bootromVersion=bootromVersion, nmInterfaceGateAddress=nmInterfaceGateAddress, gbnPlatformOAMSnmpNotifyType=gbnPlatformOAMSnmpNotifyType, cpuBusyAlarmEnable=cpuBusyAlarmEnable, sysIfMACAddr=sysIfMACAddr, sysIfBOOTPOnOff=sysIfBOOTPOnOff, softwareCompiledTimeE=softwareCompiledTimeE, snmpNotifyTypeTable=snmpNotifyTypeTable, snmpRemoteEngineTable=snmpRemoteEngineTable, controlStatus=controlStatus, loadExecute=loadExecute, loadFtpAddress=loadFtpAddress, musrType=musrType, sdramDescription=sdramDescription, writeNMInterfaceConifig=writeNMInterfaceConifig, manageUserTacacsAccount=manageUserTacacsAccount, writeNMInterfaceConifigStatus=writeNMInterfaceConifigStatus, loadExecuteStatus=loadExecuteStatus, snmpNotifyTypeEntry=snmpNotifyTypeEntry, portCarRateBpdu=portCarRateBpdu, manageUserAuthChallegeType=manageUserAuthChallegeType, cpuUnbusyThreshold=cpuUnbusyThreshold, loadFtpFileName=loadFtpFileName, vctRunEnable=vctRunEnable, gbnPlatformOAMSnmp=gbnPlatformOAMSnmp, gbnPlatformOAMWatchDog=gbnPlatformOAMWatchDog, vctRunResultErrorLocation=vctRunResultErrorLocation, snmpTrapSourceType=snmpTrapSourceType, portCarEntry=portCarEntry, snmpRemoteEngineEntry=snmpRemoteEngineEntry, gbnPlatformOAMUpDownLoad=gbnPlatformOAMUpDownLoad, gbnPlatformOAMSyslog=gbnPlatformOAMSyslog, controlTeminal=controlTeminal, manageUserAuthenType=manageUserAuthenType, snmpCommunityToViewTable=snmpCommunityToViewTable, sysIfManageVLANTbale=sysIfManageVLANTbale, manageUserTacacsAuthor=manageUserTacacsAuthor, cpuBusyThreshold=cpuBusyThreshold, gbnPlatformOAMIpAccessControl=gbnPlatformOAMIpAccessControl, discardBpdu=discardBpdu, VctRunResultTxRxPairNoType=VctRunResultTxRxPairNoType, hardwareVersion=hardwareVersion, gbnPlatformOAMMailalarm=gbnPlatformOAMMailalarm, softwareCompiledTimeC=softwareCompiledTimeC, writeConfig=writeConfig, portCarTable=portCarTable, offsetNegFlag=offsetNegFlag, gbnPlatformOAMVctRunResult=gbnPlatformOAMVctRunResult, cpuBusyStatus=cpuBusyStatus, sysIfIpAddress=sysIfIpAddress, snmpComm2ViewPermission=snmpComm2ViewPermission, timeZoneName=timeZoneName, vctRunEntry=vctRunEntry, sysIfManageVLANRowStatus=sysIfManageVLANRowStatus, flashDescription=flashDescription, portCarRate=portCarRate, cpuUnbusyTrap=cpuUnbusyTrap, VctRunResultStatusType=VctRunResultStatusType, gbnPlatformOAMSntpClient=gbnPlatformOAMSntpClient, gbnPlatformOAMSysIf=gbnPlatformOAMSysIf, snmpRemoteEngineID=snmpRemoteEngineID, productName=productName, portCarOpenTime=portCarOpenTime, snmpRemoteHostTAddr=snmpRemoteHostTAddr, ipAccessControlTable=ipAccessControlTable, manageUserAuthenRadiusName=manageUserAuthenRadiusName, musrName=musrName, gbnPlatformOAMMuser=gbnPlatformOAMMuser, vctRunResultTxRxPairNo=vctRunResultTxRxPairNo, systemClock=systemClock, cpuStatusTrap=cpuStatusTrap, vctAutoRunEnable=vctAutoRunEnable, cpuIdle=cpuIdle, softwarePlate=softwarePlate, ipAccessControlEntry=ipAccessControlEntry, musrIndex=musrIndex, gbnPlatformOAMSsh=gbnPlatformOAMSsh, snmpNotifyTypeSaveConfiguration=snmpNotifyTypeSaveConfiguration, cpuDescription=cpuDescription, vctRunResultPort=vctRunResultPort, sysIfManageVLANVid=sysIfManageVLANVid, loadFtpUserName=loadFtpUserName, saveNMInterfaceConfig=saveNMInterfaceConfig, musrEntry=musrEntry, portCarGlobalEnable=portCarGlobalEnable, snmpDeleteRemoteEngineTableRow=snmpDeleteRemoteEngineTableRow, gbnPlatformOAMPortCar=gbnPlatformOAMPortCar, sysIfIPStatus=sysIfIPStatus, nmInterfaceNetMask=nmInterfaceNetMask, snmpNotifyTypeStatus=snmpNotifyTypeStatus, hostName=hostName, snmpComm2ViewRowStatus=snmpComm2ViewRowStatus, vctRunTable=vctRunTable, sysIfIPNetMask=sysIfIPNetMask, portDiscardBpdu=portDiscardBpdu, sysIfIPGateAddress=sysIfIPGateAddress, sysIfManageVLANEntry=sysIfManageVLANEntry, snmpComm2ViewIndex=snmpComm2ViewIndex, vctRunPort=vctRunPort, memoryIdle=memoryIdle, gbnPlatformOAM=gbnPlatformOAM, snmpComm2ViewViewName=snmpComm2ViewViewName, musrTable=musrTable, softwareVersion=softwareVersion, gbnPlatformOAMVctRun=gbnPlatformOAMVctRun, systemReset=systemReset, nmInterfaceId=nmInterfaceId, musrRowStatus=musrRowStatus, nmInterfaceIpAddress=nmInterfaceIpAddress, loadTftpAddress=loadTftpAddress, controlIpAddress=controlIpAddress, controlIpMask=controlIpMask, musrPassword=musrPassword, snmpTrapSource=snmpTrapSource)
