...
# ASN.1 source file:///Users/rob/Code/pysnmp-mibs/mibs/rfc/MIP-MIB
...
mipSecViolatorAddress = MibTableColumn((1, 3, 6, 1, 2, 1, 44, 1, 2, 3, 1, 1), IpAddress()).setMaxAccess("accessiblefornotify")
...
mibBuilder.exportSymbols("MIP-MIB", mnSystem=mnSystem, mnRegFlags=mnRegFlags, maAdvMinInterval=maAdvMinInterval, mipSecAssocTable=mipSecAssocTable, mipSecPeerAddress=mipSecPeerAddress, mnAdvMaxAdvLifetime=mnAdvMaxAdvLifetime, mnGratuitousARPsSend=mnGratuitousARPsSend, faVisitorHomeAddress=faVisitorHomeAddress, mipSecurity=mipSecurity, mipSecViolationTable=mipSecViolationTable, haCounterEntry=haCounterEntry, faVisitorTimeGranted=faVisitorTimeGranted, maAdvAddress=maAdvAddress, mipMIBNotifications=mipMIBNotifications, haMobilityBindingEntry=haMobilityBindingEntry, mipMIB=mipMIB, mipSecRecentViolationTime=mipSecRecentViolationTime, faRegRepliesRelayed=faRegRepliesRelayed, mnSystemGroup=mnSystemGroup, mipSecTotalViolations=mipSecTotalViolations, mipSecViolatorAddress=mipSecViolatorAddress, faCOAStatus=faCOAStatus, faPoorlyFormedRequests=faPoorlyFormedRequests, haUnknownHA=haUnknownHA, mipSecAlgorithmMode=mipSecAlgorithmMode, faRegLifetimeTooLong=faRegLifetimeTooLong, mnDeRegRequestsSent=mnDeRegRequestsSent, faVisitorIPAddress=faVisitorIPAddress, mnRegRequestsSent=mnRegRequestsSent, mnAdvFlags=mnAdvFlags, mipSecSPI=mipSecSPI, maAdvsSentForSolicitation=maAdvsSentForSolicitation, mnRegRequestsDeniedByHADueToID=mnRegRequestsDeniedByHADueToID, mipSecRecentViolationIDHigh=mipSecRecentViolationIDHigh, mnRegRequestsWithDirectedBroadcast=mnRegRequestsWithDirectedBroadcast, faCOAEntry=faCOAEntry, faHAAuthenticationFailure=faHAAuthenticationFailure, haMobilityBindingRegFlags=haMobilityBindingRegFlags, haMobilityBindingRegIDHigh=haMobilityBindingRegIDHigh, mnRepliesInvalidHomeAddress=mnRepliesInvalidHomeAddress, mnCurrentHA=mnCurrentHA, faVisitorRegFlags=faVisitorRegFlags, mnAdvTimeReceived=mnAdvTimeReceived, mipEntities=mipEntities, mipSecRecentViolationIDLow=mipSecRecentViolationIDLow, mnAdvertisementsReceived=mnAdvertisementsReceived, mnSolicitationsSent=mnSolicitationsSent, faRegistrationRequired=faRegistrationRequired, faPoorlyFormedReplies=faPoorlyFormedReplies, mipGroups=mipGroups, faSupportedCOA=faSupportedCOA, haReasonUnspecified=haReasonUnspecified, mipHA=mipHA, haRegistrationAccepted=haRegistrationAccepted, maAdvertisementsSent=maAdvertisementsSent, faRegistrationGroup=faRegistrationGroup, faReasonUnspecified=faReasonUnspecified, haAdmProhibited=haAdmProhibited, mnRepliesIgnoredUnknownExtension=mnRepliesIgnoredUnknownExtension, mipEncapsulationSupported=mipEncapsulationSupported, mnRepliesFAAuthenticationFailure=mnRepliesFAAuthenticationFailure, mipMIBConformance=mipMIBConformance, mnRegRequestsDeniedByFA=mnRegRequestsDeniedByFA, haMultiBindingUnsupported=haMultiBindingUnsupported, mipSecAssociationGroup=mipSecAssociationGroup, mipSecViolationGroup=mipSecViolationGroup, mnRegCOA=mnRegCOA, haMobilityBindingTimeGranted=haMobilityBindingTimeGranted, mnFAAddress=mnFAAddress, faVisitorEntry=faVisitorEntry, mnAdvSequence=mnAdvSequence, faRegRequestsRelayed=faRegRequestsRelayed, haMobilityBindingRegIDLow=haMobilityBindingRegIDLow, mnRegistrationGroup=mnRegistrationGroup, haRegRepliesSent=haRegRepliesSent, mnRecentAdvReceived=mnRecentAdvReceived, faVisitorHomeAgentAddress=faVisitorHomeAgentAddress, mnRegRequestsAccepted=mnRegRequestsAccepted, mipMIBObjects=mipMIBObjects, mnRepliesUnknownFA=mnRepliesUnknownFA, faHAUnreachable=faHAUnreachable, haServiceRequestsAccepted=haServiceRequestsAccepted, faAdvertisement=faAdvertisement, mnRegRequestsDeniedByHA=mnRegRequestsDeniedByHA, mnHAAddress=mnHAAddress, maAdvConfigTable=maAdvConfigTable, mnRegTimeRequested=mnRegTimeRequested, haRegistration=haRegistration, mipFA=mipFA, maAdvPrefixLengthInclusion=maAdvPrefixLengthInclusion, mnHAStatus=mnHAStatus, mipMN=mipMN, haTooManyBindings=haTooManyBindings, haRegistrationGroup=haRegistrationGroup, mipSecKey=mipSecKey, faIsBusy=faIsBusy, haMobilityBindingTimeRemaining=haMobilityBindingTimeRemaining, mnCOA=mnCOA, haIDMismatch=haIDMismatch, haRecentServiceDeniedTime=haRecentServiceDeniedTime, mipSecViolationCounter=mipSecViolationCounter, faAdmProhibited=faAdmProhibited, haOverallServiceTime=haOverallServiceTime, mipAuthFailure=mipAuthFailure, mipSecViolationEntry=mipSecViolationEntry, mnRegAgentAddress=mnRegAgentAddress, mipSecAssocEntry=mipSecAssocEntry, mnRegIDLow=mnRegIDLow, mnHAEntry=mnHAEntry, haRegNodeCountersGroup=haRegNodeCountersGroup, mnRepliesUnknownHA=mnRepliesUnknownHA, mnAgentRebootsDectected=mnAgentRebootsDectected, haMobilityBindingTable=haMobilityBindingTable, mnMoveFromFAToFA=mnMoveFromFAToFA, haMobilityBindingSourceAddress=haMobilityBindingSourceAddress, mnMoveFromFAToHA=mnMoveFromFAToHA, faInsufficientResource=faInsufficientResource, mnRegIsAccepted=mnRegIsAccepted, faMNAuthenticationFailure=faMNAuthenticationFailure, mnHATable=mnHATable, haInsufficientResource=haInsufficientResource, mipSecRecentViolationReason=mipSecRecentViolationReason, haProxyARPsSent=haProxyARPsSent, PYSNMP_MODULE_ID=mipMIB, mnRepliesInvalidID=mnRepliesInvalidID, mipMA=mipMA, maAdvResponseSolicitationOnly=maAdvResponseSolicitationOnly, haRecentServiceAcceptedTime=haRecentServiceAcceptedTime, mnDiscovery=mnDiscovery, haPoorlyFormedRequest=haPoorlyFormedRequest, mnState=mnState, maAdvertisementGroup=maAdvertisementGroup, mnRegistration=mnRegistration, haMobilityBindingCOA=haMobilityBindingCOA, mnMoveFromHAToFA=mnMoveFromHAToFA, mnAdvMaxRegLifetime=mnAdvMaxRegLifetime, mipSecReplayMethod=mipSecReplayMethod, mipSecRecentViolationSPI=mipSecRecentViolationSPI, haMobilityBindingMN=haMobilityBindingMN, maAdvertisement=maAdvertisement, haRecentServiceDeniedCode=haRecentServiceDeniedCode, mipSecAlgorithmType=mipSecAlgorithmType, haMNAuthenticationFailure=haMNAuthenticationFailure, mnCOAIsLocal=mnCOAIsLocal, faVisitorTimeRemaining=faVisitorTimeRemaining, faEncapsulationUnavailable=faEncapsulationUnavailable, faSystem=faSystem, haDeRegRequestsReceived=haDeRegRequestsReceived, faVisitorRegIsAccepted=faVisitorRegIsAccepted, faVJCompressionUnavailable=faVJCompressionUnavailable, mipCompliance=mipCompliance, mnRegistrationEntry=mnRegistrationEntry, mipSystem=mipSystem, mnRegTimeSent=mnRegTimeSent, maAdvMaxInterval=maAdvMaxInterval, mipSecNotificationsGroup=mipSecNotificationsGroup, haGratuitiousARPsSent=haGratuitiousARPsSent, mnAdvsDroppedInvalidExtension=mnAdvsDroppedInvalidExtension, maAdvStatus=maAdvStatus, faVisitorTable=faVisitorTable, haCounterTable=haCounterTable, faAdvertisementGroup=faAdvertisementGroup, haServiceRequestsDenied=haServiceRequestsDenied, faRegistration=faRegistration, mnHomeAddress=mnHomeAddress, mnAdvsIgnoredUnknownExtension=mnAdvsIgnoredUnknownExtension, RegistrationFlags=RegistrationFlags, faRegRequestsReceived=faRegRequestsReceived, mnFATable=mnFATable, faCOATable=faCOATable, haDeRegRepliesSent=haDeRegRepliesSent, mnFAEntry=mnFAEntry, mipSystemGroup=mipSystemGroup, maAdvConfigEntry=maAdvConfigEntry, mnRepliesHAAuthenticationFailure=mnRepliesHAAuthenticationFailure, haFAAuthenticationFailure=haFAAuthenticationFailure, mnRegistrationTable=mnRegistrationTable, faRegRepliesRecieved=faRegRepliesRecieved, maSolicitationsReceived=maSolicitationsReceived, mipEnable=mipEnable, mnRegRepliesRecieved=mnRegRepliesRecieved, mnRepliesDroppedInvalidExtension=mnRepliesDroppedInvalidExtension, mipMIBNotificationPrefix=mipMIBNotificationPrefix, mnDeRegRepliesRecieved=mnDeRegRepliesRecieved, mipCompliances=mipCompliances, haRegRequestsReceived=haRegRequestsReceived, faVisitorRegIDHigh=faVisitorRegIDHigh, mnAdvSourceAddress=mnAdvSourceAddress, faSystemGroup=faSystemGroup, maAdvMaxAdvLifetime=maAdvMaxAdvLifetime, faVisitorRegIDLow=faVisitorRegIDLow, mnDiscoveryGroup=mnDiscoveryGroup, mnRegTimeRemaining=mnRegTimeRemaining, maAdvMaxRegLifetime=maAdvMaxRegLifetime, mnRegIDHigh=mnRegIDHigh, maInterfaceAddress=maInterfaceAddress)
