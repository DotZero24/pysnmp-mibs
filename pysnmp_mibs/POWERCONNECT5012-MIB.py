...
# ASN.1 source file:///Users/rob/code/pysnmp-mibs/mibs/dell/POWERCONNECT5012-MIB
...
mibBuilder.exportSymbols("POWERCONNECT5012-MIB", portTrunkMember=portTrunkMember, configIpFilterStatus=configIpFilterStatus, dell=dell, configPortSrcSecurity=configPortSrcSecurity, configRadiusSharedSecret=configRadiusSharedSecret, vlanId=vlanId, configPasswordAdmin=configPasswordAdmin, configIpFilterTable=configIpFilterTable, configRestoreDefaults=configRestoreDefaults, hostEntry=hostEntry, miscSwitchOperState=miscSwitchOperState, configPhysAddress=configPhysAddress, configPortTable=configPortTable, configPortFlowControlOper=configPortFlowControlOper, configVerSwPrimary=configVerSwPrimary, configPortRuntFilt=configPortRuntFilt, dellVlanGroup=dellVlanGroup, hostTable=hostTable, configIpFilter=configIpFilter, MacAddress=MacAddress, configPortDuplex=configPortDuplex, configDynamicAddressLearning=configDynamicAddressLearning, configPortGBIC=configPortGBIC, configMirroringOnOff=configMirroringOnOff, hostStatus=hostStatus, commSet=commSet, configNetMask=configNetMask, dellLan=dellLan, dellHostGroup=dellHostGroup, configPort=configPort, configIpAssignmentMode=configIpAssignmentMode, vlanPortPortId=vlanPortPortId, configMirrorMon=configMirrorMon, vlanPortEntry=vlanPortEntry, configDiffServDSCP=configDiffServDSCP, configTftpServerIpAddress=configTftpServerIpAddress, configDiffServTable=configDiffServTable, vlanTable=vlanTable, vlanName=vlanName, configIpFilterIpAddress=configIpFilterIpAddress, hostName=hostName, configIpAddress=configIpAddress, spanOnOff=spanOnOff, configDiffServPriority=configDiffServPriority, configTftpOperation=configTftpOperation, addressIntrusion=addressIntrusion, OwnerString=OwnerString, commGet=commGet, configMirrorSrc=configMirrorSrc, dellSpanGroup=dellSpanGroup, configForwardingMode=configForwardingMode, vlanEntry=vlanEntry, configPortEntry=configPortEntry, dellMiscGroup=dellMiscGroup, configHighPriorityOptimization=configHighPriorityOptimization, dellPortTrunkGroup=dellPortTrunkGroup, powerconnect5012=powerconnect5012, configRadiusServerIpAddress=configRadiusServerIpAddress, portTrunkEntry=portTrunkEntry, configDiffServEntry=configDiffServEntry, vlanPortStatus=vlanPortStatus, configGateway=configGateway, configPortPriority=configPortPriority, commEntry=commEntry, configPasswordUser=configPasswordUser, configPortComments=configPortComments, configTelnetConsole=configTelnetConsole, hostComm=hostComm, configBootMode=configBootMode, configRmonOnOff=configRmonOnOff, configIGMPOnOff=configIGMPOnOff, configTftpServerFileName=configTftpServerFileName, dellConfigGroup=dellConfigGroup, vlanPortVlanId=vlanPortVlanId, commStatus=commStatus, hostIndex=hostIndex, configPortFastLink=configPortFastLink, vlanPortTaggedMode=vlanPortTaggedMode, commTrap=commTrap, vlanStatus=vlanStatus, configVerHwChipSet=configVerHwChipSet, portTrunkTable=portTrunkTable, hostIP=hostIP, configPortDataRateOper=configPortDataRateOper, configPortDuplexOper=configPortDuplexOper, configPortFlowControl=configPortFlowControl, configBootFtpServerIp=configBootFtpServerIp, configBootImageFileName=configBootImageFileName, configVerBootRomImage=configVerBootRomImage, configIpFilterEntry=configIpFilterEntry, dellCommGroup=dellCommGroup, configPortDefaultVlanId=configPortDefaultVlanId, vlanPortTable=vlanPortTable, configWebOnOff=configWebOnOff, portTrunkId=portTrunkId, portTrunkMemberValue=portTrunkMemberValue, miscStatisticsReset=miscStatisticsReset, commIndex=commIndex, commName=commName, configPortAutoNegotiation=configPortAutoNegotiation, commTable=commTable, configSave=configSave, configPortStateOper=configPortStateOper, configPortHOLBlocking=configPortHOLBlocking, hostAuthorization=hostAuthorization, configPortDataRate=configPortDataRate, configVerifyPassword=configVerifyPassword, configUserAuthenticationMode=configUserAuthenticationMode, miscReset=miscReset, miscBaud=miscBaud)
