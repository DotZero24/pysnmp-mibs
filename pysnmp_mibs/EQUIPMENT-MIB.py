...
# ASN.1 source file:///Users/rob/Code/pysnmp-mibs/mibs/d-link/EQUIPMENT-MIB
# Produced by pysmi-1.1.12 at Wed Oct  8 10:59:33 2025
...
mibBuilder.exportSymbols("EQUIPMENT-MIB", swEquipTemperatureNotifyMgmt=swEquipTemperatureNotifyMgmt, swEquipment=swEquipment, swLowTemperatureRecover=swLowTemperatureRecover, swFanRecoverSeverity=swFanRecoverSeverity, swUnitStackMode=swUnitStackMode, swFanEntry=swFanEntry, swEquipmentPowerSaving=swEquipmentPowerSaving, swEquipPowerSavingHiberRowStatus=swEquipPowerSavingHiberRowStatus, swTemperatureLowThresh=swTemperatureLowThresh, swExternalAlarmStackingMessage=swExternalAlarmStackingMessage, swEquipPowerSavingPortLedTable=swEquipPowerSavingPortLedTable, swEquipUnitNotifyMgmt=swEquipUnitNotifyMgmt, swUnitMgmtPromVersion=swUnitMgmtPromVersion, swTemperatureTable=swTemperatureTable, swExternalAlarmStatus=swExternalAlarmStatus, swFanNumber=swFanNumber, swEquipFanNotifyMgmt=swEquipFanNotifyMgmt, swUnitMgmtCtrlMode=swUnitMgmtCtrlMode, swUnitMgmtPriority=swUnitMgmtPriority, swUnitMgmtTable=swUnitMgmtTable, swExternalAlarmStacking=swExternalAlarmStacking, swEquipPowerSavingScheduleCtrl=swEquipPowerSavingScheduleCtrl, swUnitRemoveSeverity=swUnitRemoveSeverity, swEquipmentLEDCtrl=swEquipmentLEDCtrl, swPowerStatusChgSeverity=swPowerStatusChgSeverity, swExternalAlarmStackingTable=swExternalAlarmStackingTable, swEquipmentNotify=swEquipmentNotify, swEquipmentMIB=swEquipmentMIB, swUnitStackTrapState=swUnitStackTrapState, swUnitStackForceMasterRole=swUnitStackForceMasterRole, swUnitMgmtEntry=swUnitMgmtEntry, swEquipPowerSavingHiberState=swEquipPowerSavingHiberState, swFanTable=swFanTable, swUnitRemove=swUnitRemove, swFanSpeed=swFanSpeed, swHighTemperatureSeverity=swHighTemperatureSeverity, swUnitMgmtUserSetState=swUnitMgmtUserSetState, swEquipmentPortLEDState=swEquipmentPortLEDState, swPowerUnitIndex=swPowerUnitIndex, swUnitMgmtVersion=swUnitMgmtVersion, swLowTemperature=swLowTemperature, swUnitMgmtModuleName=swUnitMgmtModuleName, swEquipmentNotifyMgmt=swEquipmentNotifyMgmt, swEquipPowerSavingHibernationTable=swEquipPowerSavingHibernationTable, swPowerTable=swPowerTable, swUnitNumOfUnit=swUnitNumOfUnit, swEquipPowerSavingPortRowStatus=swEquipPowerSavingPortRowStatus, swExternalAlarmTable=swExternalAlarmTable, swUnitTPChange=swUnitTPChange, swEquipmentNotification=swEquipmentNotification, swEquipPowerSavingPortLedRowStatus=swEquipPowerSavingPortLedRowStatus, swUnitInsert=swUnitInsert, swNotificationBindings=swNotificationBindings, swEquipPowerSavingPortTable=swEquipPowerSavingPortTable, swHighTemperatureRecoverSeverity=swHighTemperatureRecoverSeverity, swTemperatureHighThresh=swTemperatureHighThresh, swPowerTrapState=swPowerTrapState, swFanStatus=swFanStatus, swTemperatureTrapState=swTemperatureTrapState, MacAddress=MacAddress, swEquipTemperNotifyBindings=swEquipTemperNotifyBindings, swUnitMgmtSerialNumber=swUnitMgmtSerialNumber, swTemperatureLogState=swTemperatureLogState, swPowerStatusChg=swPowerStatusChg, swPowerRecoverSeverity=swPowerRecoverSeverity, swPowerFailure=swPowerFailure, swEquipPowerSavingHibernationEntry=swEquipPowerSavingHibernationEntry, swStackTopologyType=swStackTopologyType, swEquipUnitNotifyPrefix=swEquipUnitNotifyPrefix, swPowerID=swPowerID, swEquipFanNotification=swEquipFanNotification, swEquipmentCapacity=swEquipmentCapacity, swUnitMgmtExistState=swUnitMgmtExistState, swEquipPowerSavingPortLEDState=swEquipPowerSavingPortLEDState, swLowTemperatureRecoverSeverity=swLowTemperatureRecoverSeverity, swFanFailureSeverity=swFanFailureSeverity, swUnitMgmtMacAddr=swUnitMgmtMacAddr, swEquipPowerSavingPortState=swEquipPowerSavingPortState, swFanID=swFanID, swFanUnitIndex=swFanUnitIndex, swExternalAlarmStackingEntry=swExternalAlarmStackingEntry, swTemperatureEntry=swTemperatureEntry, swUnitMaxSupportedUnits=swUnitMaxSupportedUnits, swUnitMgmtFirmwareVersion=swUnitMgmtFirmwareVersion, swEquipPowerSavingLinkDetectState=swEquipPowerSavingLinkDetectState, swUnitMgmtFrontPanelLedStatus=swUnitMgmtFrontPanelLedStatus, swLowTemperatureSeverity=swLowTemperatureSeverity, swPowerFailureSeverity=swPowerFailureSeverity, swEquipExternalAlarmNotification=swEquipExternalAlarmNotification, swExternalAlarm=swExternalAlarm, swUnitInsertSeverity=swUnitInsertSeverity, swUnitMgmtStartPort=swUnitMgmtStartPort, PYSNMP_MODULE_ID=swEquipmentMIB, swUnitMgmt=swUnitMgmt, swUnitStackLogState=swUnitStackLogState, swEquipExternalAlarmNotifyPrefix=swEquipExternalAlarmNotifyPrefix, swExternalAlarmStackingStatus=swExternalAlarmStackingStatus, swEquipPowerSavingPortLedEntry=swEquipPowerSavingPortLedEntry, swFanFailure=swFanFailure, swUnitMgmtId=swUnitMgmtId, swPowerRecover=swPowerRecover, swUnitMgmtCurrentMode=swUnitMgmtCurrentMode, swEquipUnitNotification=swEquipUnitNotification, swEquipPowerSavingPortIndex=swEquipPowerSavingPortIndex, swEquipTemperatureNotification=swEquipTemperatureNotification, swEquipPowerSavingLenDetect=swEquipPowerSavingLenDetect, swExternalAlarmEntry=swExternalAlarmEntry, swTemperatureCurrent=swTemperatureCurrent, swEquipmentTemperatureCtrl=swEquipmentTemperatureCtrl, swUnitMgmtHardwareVersion=swUnitMgmtHardwareVersion, swUnitMgmtPortRange=swUnitMgmtPortRange, swExternalAlarmChannel=swExternalAlarmChannel, swUnitRoleChange=swUnitRoleChange, swEquipFanNotifyPrefix=swEquipFanNotifyPrefix, swFanPostion=swFanPostion, swExternalAlarmStackingChannel=swExternalAlarmStackingChannel, swStackRoleChangeType=swStackRoleChangeType, swEquipPowerNotifyMgmt=swEquipPowerNotifyMgmt, swEquipUnitNotifyBindings=swEquipUnitNotifyBindings, swHighTemperature=swHighTemperature, swEquipTemperatureNotifyPrefix=swEquipTemperatureNotifyPrefix, swEquipPowerNotification=swEquipPowerNotification, swFanRecover=swFanRecover, swEquipEeeState=swEquipEeeState, swHighTemperatureRecover=swHighTemperatureRecover, swUnitFailureSeverity=swUnitFailureSeverity, swTemperatureUnitIndex=swTemperatureUnitIndex, swTemperSensorID=swTemperSensorID, swExternalAlarmStackingUnitIndex=swExternalAlarmStackingUnitIndex, swEquipPowerNotifyPerfix=swEquipPowerNotifyPerfix, swPowerStatus=swPowerStatus, swEquipEeeMgmt=swEquipEeeMgmt, swPowerEntry=swPowerEntry, swUnitStackingVersion=swUnitStackingVersion, swUnitFailure=swUnitFailure, swEquipPowerSavingPortEntry=swEquipPowerSavingPortEntry, swUnitTopology=swUnitTopology, swUnitMgmtBoxId=swUnitMgmtBoxId, swExternalAlarmMessage=swExternalAlarmMessage, swFanTrapState=swFanTrapState)
