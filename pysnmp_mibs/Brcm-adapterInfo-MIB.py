...
# ASN.1 source file:///Users/rob/code/pysnmp-mibs/mibs/broadcom/Brcm-adapterInfo-MIB
...
ifiNumber = MibScalar((1, 3, 6, 1, 4, 1, 4413, 1, 3, 1), Integer32()).setMaxAccess("readonly")
if mibBuilder.loadTexts: ifiNumber.setStatus('mandatory')
ifiTable = MibTable((1, 3, 6, 1, 4, 1, 4413, 1, 3, 2), )
if mibBuilder.loadTexts: ifiTable.setStatus('mandatory')
ifiEntry = MibTableRow((1, 3, 6, 1, 4, 1, 4413, 1, 3, 2, 1), ).setIndexNames((0, "Brcm-adapterInfo-MIB", "ifiIndex"))
if mibBuilder.loadTexts: ifiEntry.setStatus('mandatory')
ifiIndex = MibTableColumn((1, 3, 6, 1, 4, 1, 4413, 1, 3, 2, 1, 1), Integer32().subtype(subtypeSpec=ValueRangeConstraint(0, 65535)))
...
ifiDescr = MibTableColumn((1, 3, 6, 1, 4, 1, 4413, 1, 3, 2, 1, 3), DisplayString()).setMaxAccess("readonly")
...
ifiPhysAddress = MibTableColumn((1, 3, 6, 1, 4, 1, 4413, 1, 3, 2, 1, 6), PhysAddress()).setMaxAccess("readonly")
...
mibBuilder.exportSymbols("Brcm-adapterInfo-MIB", ifiIndex=ifiIndex, ifMemBaseLow=ifMemBaseLow, ifPermPhysAddress=ifPermPhysAddress, ifInterrupt=ifInterrupt, ifDeviceNumber=ifDeviceNumber, ifiEntry=ifiEntry, ifIpv6NetworkAddress=ifIpv6NetworkAddress, ifControllers=ifControllers, baspStat=baspStat, ifMemBaseHigh=ifMemBaseHigh, baspTrap=baspTrap, baspConfig=baspConfig, ifiTable=ifiTable, ifState=ifState, basp=basp, ifiPhysAddress=ifiPhysAddress, enet=enet, ifiNumber=ifiNumber, ifNetworkAddress=ifNetworkAddress, ifSubnetMask=ifSubnetMask, ifName=ifName, ifLineSpeed=ifLineSpeed, broadcom=broadcom, ifBusNumber=ifBusNumber, ifLinkStatus=ifLinkStatus, ifiDescr=ifiDescr, ifFunctionNumber=ifFunctionNumber, ifDuplexMode=ifDuplexMode)
