...
mibBuilder.exportSymbols("AVAYA-RTP-MIB", avRtpSessionRxRtcp=avRtpSessionRxRtcp, avRtpSumActiveFlows=avRtpSumActiveFlows, avRtpThresholdRxLoss=avRtpThresholdRxLoss, avRtpQoSFaultTrapEnable=avRtpQoSFaultTrapEnable, avRtpSessionTxLen=avRtpSessionTxLen, avRtpThresholdRxCodecLoss=avRtpThresholdRxCodecLoss, avRtpLookupEntry=avRtpLookupEntry, avRtpThresholdRxAvgCodecLoss=avRtpThresholdRxAvgCodecLoss, avRtpSessionRemAvgJitter=avRtpSessionRemAvgJitter, avRtpSessionAvgCodecRtt=avRtpSessionAvgCodecRtt, avRtpSessionTxEncryp=avRtpSessionTxEncryp, avRtpSessionRtt=avRtpSessionRtt, avRtpSessionAvgRtt=avRtpSessionAvgRtt, avRtpQoSFault=avRtpQoSFault, avRtpSumClear=avRtpSumClear, avRtpSumTable=avRtpSumTable, avRtpSessionLocAddrType=avRtpSessionLocAddrType, avRtpSessionRxSsrc=avRtpSessionRxSsrc, avRtpSessionDuration=avRtpSessionDuration, avRtpThresholdCodecRttEv=avRtpThresholdCodecRttEv, avRtpThresholdRxJitter=avRtpThresholdRxJitter, avRtpThresholdRemLossEv=avRtpThresholdRemLossEv, avRtpSessionRemInetAddrType=avRtpSessionRemInetAddrType, avRtpSessionRemPort=avRtpSessionRemPort, avRtpSessionRxSeqFall=avRtpSessionRxSeqFall, avRtpLookupTable=avRtpLookupTable, avRtpQoSTrapTokenInterval=avRtpQoSTrapTokenInterval, avRtpSessionRemAddrV4=avRtpSessionRemAddrV4, avRtpSessionRxSilenceSupp=avRtpSessionRxSilenceSupp, avRtpSessionState=avRtpSessionState, avRtpSessionRxLossCount=avRtpSessionRxLossCount, avRtpSessionTxSilenceSupp=avRtpSessionTxSilenceSupp, avRtpSessionTxRsvp=avRtpSessionTxRsvp, avRtpThresholdEcReturnLossEv=avRtpThresholdEcReturnLossEv, AvRtpLoss=AvRtpLoss, avRtpDateAndTime=avRtpDateAndTime, avRtpSessionRxJitterEv=avRtpSessionRxJitterEv, mibs=mibs, avRtpSumTotalFlows=avRtpSumTotalFlows, avRtpThresholdRttEv=avRtpThresholdRttEv, avRtpThresholdEcReturnLoss=avRtpThresholdEcReturnLoss, avRtpSessionRxDscp=avRtpSessionRxDscp, avRtpThresholdRxJitterEv=avRtpThresholdRxJitterEv, avRtpSessionRxTtl=avRtpSessionRxTtl, avRtpSessionEngineID=avRtpSessionEngineID, avRtpSessionRxLossEv=avRtpSessionRxLossEv, avRtpSumTxTTL=avRtpSumTxTTL, avRtpThresholdRxLossEv=avRtpThresholdRxLossEv, avRtpSessionRxCodecLossCount=avRtpSessionRxCodecLossCount, avRtpSessionRemAddrV6=avRtpSessionRemAddrV6, avRtpSessionPhone=avRtpSessionPhone, avRtpFaultMask=avRtpFaultMask, PYSNMP_MODULE_ID=avRtpMib, avRtpSumPeriod=avRtpSumPeriod, avRtpSessionRxJBufUnderruns=avRtpSessionRxJBufUnderruns, avRtpThresholdTable=avRtpThresholdTable, avRtpSumEntry=avRtpSumEntry, avRtpQoSInetTrap=avRtpQoSInetTrap, avRtpSumSessionDuration=avRtpSumSessionDuration, avRtpQoSTrap=avRtpQoSTrap, avRtpThresholdMinStatWin=avRtpThresholdMinStatWin, avRtpSessionID=avRtpSessionID, avRtpSessionRemAvgLoss=avRtpSessionRemAvgLoss, avRtpSessionStartTime=avRtpSessionStartTime, avRtpSessionRxCodecPlayTime=avRtpSessionRxCodecPlayTime, avRtpSessionRemAddrType=avRtpSessionRemAddrType, avRtpEnable=avRtpEnable, avRtpSessionRxJitter=avRtpSessionRxJitter, avRtpSessionRemAddr=avRtpSessionRemAddr, avRtpSessionType=avRtpSessionType, avRtpSessionEcReturnLossEv=avRtpSessionEcReturnLossEv, avRtpSessionRxDup=avRtpSessionRxDup, avRtpSessionRxLoss=avRtpSessionRxLoss, avRtpSessionRxMaxJBufDelay=avRtpSessionRxMaxJBufDelay, avRtpSessionStatInterval=avRtpSessionStatInterval, avRtpSessionAEC=avRtpSessionAEC, avRtpSessionLocInetAddrType=avRtpSessionLocInetAddrType, avRtpThresholdRemJitter=avRtpThresholdRemJitter, AvRtpSilenceSupp=AvRtpSilenceSupp, avRtpMaxSessionTableSize=avRtpMaxSessionTableSize, avRtpThresholdSet=avRtpThresholdSet, avRtpSessionDebugStr=avRtpSessionDebugStr, avRtpSessionRemLoss=avRtpSessionRemLoss, avRtpSessionCname=avRtpSessionCname, avRtpQoSClear=avRtpQoSClear, avRtpSessionRxMinTtl=avRtpSessionRxMinTtl, avRtpSessionTxDscp=avRtpSessionTxDscp, avRtpSessionRxAvgLoss=avRtpSessionRxAvgLoss, avRtpNotificationVarbinds=avRtpNotificationVarbinds, avRtpSessionRxCodecLossEv=avRtpSessionRxCodecLossEv, avRtpQoSTrapsDrop=avRtpQoSTrapsDrop, avRtpSessionRx=avRtpSessionRx, avRtpThresholdRemJitterEv=avRtpThresholdRemJitterEv, avRtpSessionTxFlowLabel=avRtpSessionTxFlowLabel, avRtpSessionRemJitter=avRtpSessionRemJitter, avRtpSessionRxCodecLoss=avRtpSessionRxCodecLoss, avRtpSessionRxMaxTtl=avRtpSessionRxMaxTtl, avRtpSessionTable=avRtpSessionTable, avRtpSessionLocPort=avRtpSessionLocPort, avRtpSessionLocAddr=avRtpSessionLocAddr, avRtpSessionRxJBufOverruns=avRtpSessionRxJBufOverruns, avRtpQoSTrapBucketSize=avRtpQoSTrapBucketSize, avRtpSessionTxSsrc=avRtpSessionTxSsrc, avRtpSumEngineID=avRtpSumEngineID, avRtpSumEngineDescr=avRtpSumEngineDescr, avRtpSessionLocAddrV4=avRtpSessionLocAddrV4, avRtpThresholdRemLoss=avRtpThresholdRemLoss, avRtpSessionEcTailLen=avRtpSessionEcTailLen, avRtpLookupStartTime=avRtpLookupStartTime, avRtpConfig=avRtpConfig, avRtpSessionTxL2Pri=avRtpSessionTxL2Pri, avRtpNotification=avRtpNotification, avRtpSessionRemLossEv=avRtpSessionRemLossEv, avRtpThresholdRxCodecLossEv=avRtpThresholdRxCodecLossEv, avRtpSessionEndTime=avRtpSessionEndTime, avRtpSessionRemJitterEv=avRtpSessionRemJitterEv, avRtpSessionEcReturnLoss=avRtpSessionEcReturnLoss, avRtpSumActiveQosEvents=avRtpSumActiveQosEvents, avRtpSessionRxJBufDelay=avRtpSessionRxJBufDelay, avRtpClear=avRtpClear, avRtpSessionRxSsrcChange=avRtpSessionRxSsrcChange, avRtpSessionTxInterval=avRtpSessionTxInterval, avRtpTxQoSTraps=avRtpTxQoSTraps, avRtpSessionSeverity=avRtpSessionSeverity, avRtpThresholdAvgRxLoss=avRtpThresholdAvgRxLoss, avRtpQoSClearTh=avRtpQoSClearTh, avRtpThresholdRxSsrcChangeEv=avRtpThresholdRxSsrcChangeEv, avRtpMib=avRtpMib, avRtpSessionRxL2Pri=avRtpSessionRxL2Pri, avRtpSessionRxFlowLabel=avRtpSessionRxFlowLabel, avRtpThresholdEntry=avRtpThresholdEntry, avRtpThresholdCodecRtt=avRtpThresholdCodecRtt, avRtpSessionLocInetAddr=avRtpSessionLocInetAddr, avRtpReservedSessionTableRows=avRtpReservedSessionTableRows, avRtpSessionRemInetAddr=avRtpSessionRemInetAddr, AvRtpItuPerceivedSeverity=AvRtpItuPerceivedSeverity, avRtpSessionRxAvgJitter=avRtpSessionRxAvgJitter, avRtpSessionTxRtcp=avRtpSessionTxRtcp, avRtpSessionRttEv=avRtpSessionRttEv, avaya=avaya, avRtpSumTotalFlowsQoSEvents=avRtpSumTotalFlowsQoSEvents, avRtpSessionCodecRtt=avRtpSessionCodecRtt, avRtpThresholdRtt=avRtpThresholdRtt, avRtpSessionCodecRttEv=avRtpSessionCodecRttEv, avRtpSessionRxAvgCodecLoss=avRtpSessionRxAvgCodecLoss, avRtpQoSFaultTh=avRtpQoSFaultTh, avRtpQoSTrapEnable=avRtpQoSTrapEnable, avRtpSessionTxVlan=avRtpSessionTxVlan, avRtpThresholdAvgRemLoss=avRtpThresholdAvgRemLoss, avRtpSessionTxResvFail=avRtpSessionTxResvFail, avRtpSessionEntry=avRtpSessionEntry, avRtpSessionLocAddrV6=avRtpSessionLocAddrV6)
